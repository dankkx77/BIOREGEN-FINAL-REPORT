...
xlabel('\DeltaG (\gamma) (kcal/mol)');
ylabel('Bimolecular Rate Constant, k (M^{-1}s^{-1})');
lgd2 = legend([p5, p3], 'Location', 'southeast');
set(gca, 'YScale', 'log')
ylim([1e-1 1e8]); 
xlim([-20 5])
set(gca,'xDir','reverse')
...
xlabel('\DeltaG (\gamma) (kcal/mol)');
ylabel('Bimolecular Rate Constant, k (M^{-1}s^{-1})');
lgd3 = legend([p6, p4], 'Location', 'southeast');
set(gca, 'YScale', 'log')
ylim([1e-1 1e8]); 
xlim([-20 5])
set(gca,'xDir','reverse')
...
grid on
% Plot Model Equation for a incumbent toehold length of 2
p11 = plot(G, k, 'b-', 'LineWidth', 1.5, 'MarkerSize', 6,'DisplayName','Model for \Delta G(\beta^2) = -2.7 kcal/mol');
%Plot theoretical rate constants of strands with associated Free Energies
