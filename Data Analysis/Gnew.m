...
% Concentrations from Excel
data_table = readtable("Full System (New Strands).xlsx", Sheet= 'Analysis_2', Range = 'N2:BU1034');
...
t_table = readtable("Full System (New Strands).xlsx", Sheet = 'Analysis_2', Range = 'B2:B1034');
...
for i = 1:length(totaldata(:,1)) 
    ...
    time = t(inj:nuke);
    % Define a small random noise
    epsilon = 1e-6 * randn(size(time));
    
    % Perturb time values slightly so there are no duplicate values - sort
    % to ensure strictly increasing time values
    time = sort(time + epsilon);
